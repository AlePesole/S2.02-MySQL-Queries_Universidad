...
SELECT a.nombre, ce.anyo_inicio, ce.anyo_fin FROM asignatura a
...
SELECT d.nombre AS departamento, COUNT(p.id) AS total FROM persona p
JOIN profesor prof ON p.id = prof.id_profesor
JOIN departamento d ON prof.id_departamento = d.id
...
SELECT g.nombre AS grau, a.tipo, SUM(a.creditos) AS total_creditos FROM grado g 
...
SELECT p.id, p.nombre, p.apellido1, p.apellido2, COUNT(a.id) as total FROM persona p
JOIN profesor prof ON p.id = prof.id_profesor
LEFT JOIN asignatura a ON prof.id_profesor = a.id_profesor
